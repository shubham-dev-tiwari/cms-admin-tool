...
export const db = {
  ...
  getSheets: async () => {
    ...
    // Ensure we have access to sheet info
  ...
  getAll: async (sheetTitle = 'Sheet1') => {
    ...
    if (!sheet) {
      // Try reloading info if sheet not found
      await doc.loadInfo();
      const retrySheet = doc.sheetsByTitle[sheetTitle];
      if (!retrySheet) throw new Error(`Sheet "${sheetTitle}" not found`);
      return db.getAll(sheetTitle); // Retry with reloaded info
    }
